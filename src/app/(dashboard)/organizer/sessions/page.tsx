// src/app/(dashboard)/organizer/sessions/page.tsx - CORRECTED: Session editing with proper error handling
"use client";
...
// CORRECTED: Extended DraftSession type with all required fields
type DraftSession = {
  place: string;
  roomId?: string;
  startTime?: string;
...
const AllSessions: React.FC = () => {
  ...
  // CORRECTED: Add error state for better debugging
  ...
  // CORRECTED: Separate event loading function with proper API response handling
  const loadEvents = async () => {
    try {
      ...
      // CORRECTED: Handle the correct API response format
      let eventsArray = [];
      ...
      // CORRECTED: Process events with proper field mapping
      ...
      // CORRECTED: Fallback events only if API fails
  ...
  // CORRECTED: Edit handlers with better error handling and validation
  const onEdit = (id: string) => {
    ...
    console.log("Starting edit for session:", row);
  ...
  // CORRECTED: Improved save function with better validation and error handling
  const onSave = async (id: string) => {
    const body = draft[id];
    const originalSession = sessions.find(s => s.id === id);
    
    if (!body) {
      setError("No draft data found for this session");
      return;
    }

    if (!originalSession) {
      setError(`Original session with ID ${id} not found`);
      return;
    }

    console.log("Saving session:", id, body);

    // CORRECTED: Validate required fields
    if (!body.place?.trim()) {
    ...
    // CORRECTED: Better datetime validation
    try {
      ...
      if (body.endTime && body.endTime.length === 16) {
        ...
        isoEndTime = endDate.toISOString();
      }
    } catch (error) {
    ...
    // CORRECTED: Build comprehensive payload
    const payload = {
      ...
      // CORRECTED: Include essential session fields
      title: body.title || originalSession.title,
      facultyName: body.facultyName || originalSession.facultyName,
      email: body.email || originalSession.email,
      eventId: body.eventId || originalSession.eventId,
    ...
    try {
      console.log("Sending PATCH request to /api/sessions/" + id, payload);
      ...
      if (!res.ok) {
        const errorMessage = responseData?.error || responseData?.message || `Server error: ${res.status} ${res.statusText}`;
      ...
      await load(false); // Reload data
      onCancel(id); // Exit edit mode
      setError(""); // Clear any errors
      
      // Show success feedback
      const successMessage = "Session updated successfully";
      console.log(successMessage);
      
    } catch (networkError) {
      console.error("Network error during session update:", networkError);
  ...
  // CORRECTED: Improved delete function
  const onDelete = async (id: string) => {
    ...
    if (!confirm(`Are you sure you want to delete the session "${session.title}"?`)) return;
      ...
      <div className="min-h-screen bg-gray-950 py-6">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mb-8">
            ...
            {/* Event Selection - CORRECTED: Show loading state and better error handling */}
