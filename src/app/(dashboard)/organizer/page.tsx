...
import { useSessions, useTodaysSessions, useUpcomingSessions } from '@/hooks/use-sessions';
import { useRegistrations, usePendingRegistrations, useRegistrationStats } from '@/hooks/use-registrations';
...
export default function OrganizerDashboardPage() {
  ...
  const { data: allRegistrations, isLoading: registrationsLoading } = useRegistrations();
  ...
  const handleEventClick = (eventId: string) => router.push(`/organizer/events/${eventId}`);
  ...
  const completedEvents = events?.data?.events?.filter(e => e.status === 'COMPLETED').length || 0;
  ...
  const approvedRegistrations = allRegistrations?.data?.registrations?.filter(r => r.status === 'APPROVED').length || 0;
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button variant="outline" onClick={() => router.push('/organizer/settings')}>
        ...
        {unreadNotifications > 0 && (
          <Alert className="border-blue-200 bg-blue-50">
            ...
            <AlertDescription>
              ...
              <Button variant="link" className="p-0 ml-1 h-auto" onClick={() => router.push('/organizer/notifications')}>
                View all
              </Button>
            </AlertDescription>
          </Alert>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-muted-foreground mt-1">
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-blue-600">Upcoming: {upcomingEvents}</span>
                </div>
              </div>
          ...
          {/* Faculty Management */}
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={handleManageFaculty}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Faculty Network</CardTitle>
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-muted-foreground mt-1">
                <div className="flex items-center space-x-4">
                  <span className="text-green-600">Active: {activeFaculty}</span>
                  {pendingFacultyInvitations > 0 && (
                    <span className="text-orange-600">Pending: {pendingFacultyInvitations}</span>
                  )}
                </div>
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
                <Star className="h-3 w-3 mr-1 text-yellow-500" />
                Faculty engagement: {facultyStats?.data?.activationRate?.toFixed(1) || 0}%
              </div>
            </CardContent>
          </Card>

          {/* Registration Analytics */}
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={handleViewRegistrations}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-muted-foreground mt-1">
                <div className="flex items-center space-x-4">
                  <span className="text-green-600">Approved: {approvedRegistrations}</span>
                  {pendingCount > 0 && (
              ...
              <div className="flex items-center text-xs text-muted-foreground">
                ...
                Registration rate: {registrationStats?.data?.registrationRate?.toFixed(1) || 0}%
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <CardContent>
              {eventsLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm mb-4">Start by creating your first conference event</p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              ) : todaysSessions?.data?.sessions?.length > 0 ? (
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {todaysSessions?.data?.sessions?.slice(0, 6).map((session) => (
                    <div 
                      key={session.id} 
                      className="p-3 border rounded text-sm cursor-pointer hover:bg-gray-50"
                      onClick={() => router.push(`/organizer/sessions/${session.id}`)}
                    >
            ...
            <CardContent className="space-y-3">
              {pendingCount > 0 && (
                <div className="flex items-center justify-between p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-orange-600">{pendingCount} awaiting review</p>
                  </div>
                  <Button 
                    size="sm" 
                    variant="outline" 
              ...
              {pendingFacultyInvitations > 0 && (
                <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-blue-600">{pendingFacultyInvitations} pending responses</p>
                  </div>
                  <Button 
                    size="sm" 
                    variant="outline" 
              ...
              {upcomingEvents > 0 && (
                <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div>
                    <h5 className="font-medium text-green-800">Upcoming Events</h5>
                    <p className="text-xs text-green-600">{upcomingEvents} events need attention</p>
                  </div>
                  <Button 
                    size="sm" 
                    variant="outline" 
              ...
              {pendingCount === 0 && pendingFacultyInvitations === 0 && upcomingEvents === 0 && (
                <div className="text-center py-6 text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${Math.round((completedEvents / (totalEvents || 1)) * 100)}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 text-center">
                ...
                <div>
                  <div className="text-lg font-bold text-purple-600">{approvedRegistrations}</div>
