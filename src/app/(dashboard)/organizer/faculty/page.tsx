...
export default function FacultyManagementPage() {
  ...
  // Load events and existing faculty data on component mount
  useEffect(() => {
  ...
  const loadEventsAndFacultyData = async () => {
    try {
      ...
      // For demo purposes, create sample events. In production, fetch from API
      const sampleEvents: Event[] = [
        ...
        {
          id: "event-2",
          name: "International Research Symposium 2025",
          startDate: new Date(
        ...
        {
          id: "event-3",
          name: "Technology Innovation Summit 2025",
          startDate: new Date(
      ...
      // Load existing faculty data from localStorage
  ...
  // Improved column detection functions
  const detectName = (row: any, index: number): string => {
    ...
    // If still no name found, check if there's any meaningful text in first few columns
    ...
    for (const value of possibleTextFields) {
      if (
        ...
        // Likely a name if it's text without @ symbol
    ...
    throw new Error(
      ...
      Supported column names: Name, Full Name, First Name, Faculty Name, etc.
      Current columns found: ${Object.keys(row).join(", ")}`
    );
  };
  ...
  const detectEmail = (row: any, index: number): string => {
    ...
    // Last resort: look for any field containing @ symbol
    ...
    throw new Error(
      ...
      Supported column names: Email, E-mail, Email Address, Mail, etc.
      Current columns found: ${Object.keys(row).join(", ")}`
    );
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Fix for TypeScript error: Type 'undefined' cannot be used as an index type
          const sheetNames = workbook.SheetNames;
          if (!sheetNames || sheetNames.length === 0) {
            throw new Error("Excel file has no worksheets");
          }

          const firstSheetName = sheetNames[0]; // Now guaranteed to be string
          if (!firstSheetName) {
            throw new Error("No worksheet name found");
          }
          ...
          if (!Array.isArray(jsonData) || jsonData.length === 0) {
            throw new Error(
              "Excel file is empty or contains no data rows. Please ensure your Excel file has data below the header row."
            );
          }
          ...
          const facultyMembers: FacultyMember[] = jsonData.map(
            (row: any, index: number) => {
              try {
                ...
                // Detect optional fields with flexible column names
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.log(
        `📊 Processed ${facultyMembers.length} faculty members from Excel`
      );

      // Remove duplicates within the new upload
      ...
      console.log(
        `✅ ${uniqueMembers.length} unique faculty members after deduplication`
      );

      // Check for existing faculty for this event
      ...
      console.log(
        `✅ Upload completed: ${addedCount} new, ${finalFacultyList.length} total`
      );
    } catch (error) {
                ...
                <div className="text-xs text-gray-600 space-y-2">
                  <div>
                    ...
                    <ul className="list-disc ml-4 space-y-1">
                      ...
                      <li>
                        <strong>Email:</strong> Email, E-mail, Email Address,
                        Mail, etc.
                      </li>
                    </ul>
                  </div>
                  <div>
                    ...
                    <ul className="list-disc ml-4 space-y-1">
                      <li>
                        <strong>Department:</strong> Department, Dept, Division,
                        etc.
                      </li>
                      <li>
                        <strong>Institution:</strong> Institution, University,
                        College, etc.
                      </li>
                      <li>
                        <strong>Expertise:</strong> Expertise, Specialization,
                        Subject, etc.
                      </li>
                      <li>
                  ...
                  <div className="bg-blue-100 p-2 rounded mt-2">
                    ...
                    <p className="text-blue-700">
                      Our system automatically detects column names in various
