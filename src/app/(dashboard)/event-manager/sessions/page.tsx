// src/app/(dashboard)/event-manager/sessions/page.tsx
'use client';
...
import { useSessionsByEvent, useDeleteSession, useSessionStats } from '@/hooks/use-sessions';
...
const SESSION_STATUS = {
  ...
  ONGOING: { label: 'Ongoing', color: 'bg-green-100 text-green-800', icon: CheckCircleIcon },
...
export default function SessionsPage() {
  ...
  const [viewMode, setViewMode] = useState<'table' | 'grid' | 'schedule'>('table');
  ...
  const { data: sessionsData, isLoading: sessionsLoading, refetch } = useSessionsByEvent(
    selectedEvent, 
    {
      search: searchQuery,
  ...
  const filteredSessions = useMemo(() => {
    return sessions.filter(session => {
      ...
      if (filterStatus) {
        ...
        const status = now < startTime ? 'UPCOMING' : 
                      now >= startTime && now <= endTime ? 'ONGOING' : 'COMPLETED';
              ...
              {filteredSessions.map((session) => {
                ...
                return (
                  <TableRow key={session.id} className="hover:bg-gray-50">
                    ...
                    <TableCell>
                      <Badge className={SESSION_TYPE_COLORS[session.sessionType as keyof typeof SESSION_TYPE_COLORS]}>
                        {session.sessionType}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        {session.speakers.length > 0 ? (
                          <>
                            <div className="font-medium">{session.speakers[0].user.name}</div>
                            {session.speakers.length > 1 && (
                              <div className="text-gray-600">+{session.speakers.length - 1} more</div>
                            )}
                          </>
                        ) : (
