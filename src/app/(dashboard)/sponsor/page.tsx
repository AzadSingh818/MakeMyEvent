...
//   // Calculate stats
//   const activeSponsorships = mySponsorships?.data?.filter(s => s.status === 'ACTIVE') || [];
//   // Use the first active sponsorship's id, or an empty string if none
//   const { data: visibilityData, isLoading: visibilityLoading } = useSponsorshipVisibility(
//     activeSponsorships[0]?.id || ''
//   );
//   const totalInvestment = activeSponsorships.reduce((sum, s) => sum + (s.package?.price || 0), 0);
//   const totalReach = visibilityData?.data?.reach || 0;
