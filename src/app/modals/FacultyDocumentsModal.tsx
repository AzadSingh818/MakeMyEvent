...
export default function FacultyDocumentsModal({ isOpen, onClose, facultyId }: Props) {
  ...
  const fetchFiles = async () => {
    ...
    try {
      ...
      const cvFile = allCvFiles.length > 0 
        ? allCvFiles.sort((a: FileItem, b: FileItem) => 
  ...
  const handleDelete = async (fileId: string, fileType: 'cv' | 'presentation') => {
      ...
      try {
        if (contentType && contentType.includes("application/json")) {
          ...
          switch (response.status) {
            case 404:
              errorMessage = 'Delete endpoint not found. Check server configuration.';
              break;
            case 405:
              errorMessage = 'Delete method not allowed. Server may need DELETE method implementation.';
              break;
            case 500:
  ...
  const getFileName = (file: FileItem, index: number, type: 'cv' | 'presentation') => {
            ...
            <div className="mt-4 space-y-8">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 space-y-1">
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Uploaded by:</span> {cv.faculty?.name || 'Unknown'}
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Date:</span> {formatDate(cv.uploadedAt)}
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Size:</span> {getDisplayFileSize(cv)} MB
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Type:</span> {getFileType(cv)}
                          </p>
                        </div>
                      </div>
              ...
              <section>
                ...
                {presentations.length > 0 ? (
                        ...
                        >
                          <div className="p-4 bg-gray-50 flex justify-between items-start">
                            <div className="flex-1">
                              ...
                              <div className="mt-2 space-y-1">
                                <p className="text-sm text-gray-600">
                                  <span className="font-medium">Uploaded by:</span> {file.faculty?.name || 'Unknown'}
                                </p>
                                <p className="text-sm text-gray-600">
                                  <span className="font-medium">Date:</span> {formatDate(file.uploadedAt)}
                                </p>
                                <p className="text-sm text-gray-600">
                                  <span className="font-medium">Size:</span> {getDisplayFileSize(file)} MB
                                </p>
                                <p className="text-sm text-gray-600">
                                  <span className="font-medium">Type:</span> {getFileType(file)}
                                </p>
                                {file.session && (
                                  <p className="text-sm text-gray-600">
                                    <span className="font-medium">Session:</span> {file.session.title}
                                  </p>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex gap-2 ml-4">
                              ...
                              <button
                                onClick={() => handleDelete(file.id, 'presentation')}
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                    <Upload className="h-12 w-12 text-gray-400 mx-auto mb-2" />
                    <p className="text-gray-500">No presentations uploaded yet.</p>
                  </div>
                )}
              </section>
            </div>
