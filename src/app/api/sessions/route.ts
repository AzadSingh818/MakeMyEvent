...
// src/app/api/sessions/route.ts - COMPLETE: With proper date handling AND your existing email system
...
import { sendInviteEmail } from "../_utils/session-email"; // ✅ Your existing email function

// ✅ ENHANCED: Complete datetime parser with comprehensive logging
function parseLocalDateTime(dateTimeStr?: string): string | null {
  ...
  console.log(`🔍 parseLocalDateTime processing: "${dateTimeStr}" (type: ${typeof dateTimeStr})`);
  ...
  try {
    ...
    if (!isNaN(testDate.getTime())) {
      ...
      console.log(`✅ Parsed via Date constructor: ${dateTimeStr} -> ${isoString}`);
...
async function checkSessionConflicts(
  ...
  try {
    ...
    console.log(`🔍 Checking conflicts for session: ${sessionData.startTime} - ${sessionData.endTime}`);
      ...
      if (hasTimeOverlap) {
        if (existingSession.facultyId === sessionData.facultyId) {
          conflicts.push({
            ...
            type: "faculty",
            message: `Faculty conflict with "${existingSession.title || "Untitled Session"}"`,
        ...
        if (existingSession.hallId === sessionData.roomId) {
          conflicts.push({
            ...
            endTime: existingSession.endTime,
            type: "room",
            message: `Room conflict with "${existingSession.title || "Untitled Session"}"`,
            sessionTitle: existingSession.title || "Untitled Session",
          });
        }
      }
...
// GET: Enhanced session listing with proper date formatting
  ...
  try {
    ...
    const enriched = sessions.map((s) => {
      ...
      try {
        ...
        console.warn(`⚠️ Error calculating duration for session ${s.id}:`, error);
      }
      ...
      try {
        if (s.startTime) {
          ...
          if (!isNaN(startDate.getTime())) {
            formattedStartTime = startDate.toLocaleDateString('en-GB', {
              year: 'numeric',
              month: '2-digit', 
              day: '2-digit'
        ...
        if (s.endTime) {
          ...
          if (!isNaN(endDate.getTime())) {
            formattedEndTime = endDate.toLocaleDateString('en-GB', {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit'
...
// ✅ ENHANCED: POST handler with complete date processing AND your existing email system
export async function POST(req: NextRequest) {
  try {
    ...
    const eventId = formData.get("eventId")?.toString()?.trim() || "default-conference-2025";
    const status = (formData.get("status")?.toString()?.trim() as "Draft" | "Confirmed") || "Draft";
    const inviteStatus = formData.get("inviteStatus")?.toString()?.trim() || formData.get("invite_status")?.toString()?.trim() || "Pending";
    
    const travel = formData.get("travel")?.toString()?.trim() || formData.get("travelStatus")?.toString()?.trim() || "";
    const accommodation = formData.get("accommodation")?.toString()?.trim() || "";

    const travelRequired = travel === "yes" || travel === "true";
    const accommodationRequired = accommodation === "yes" || accommodation === "true";

    console.log("📋 Extracted session data:");
    console.log({
      title, facultyId, email, place, roomId, eventId, status, inviteStatus,
      startTime, endTime, travel: travelRequired, accommodation: accommodationRequired
    });
    ...
    console.log(`   Received startTime: "${startTime}" (length: ${startTime.length})`);
    console.log(`   Received endTime: "${endTime}" (length: ${endTime.length})`);
    ...
    try {
      if (!startTime || !endTime) {
        ...
        console.log("✅ Generated fallback times:", { finalStartTime, finalEndTime });
      } else {
      ...
      if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        throw new Error(`Invalid date objects created. Start: ${finalStartTime}, End: ${finalEndTime}`);
      }
      ...
      console.log(`   Readable duration: ${Math.round(durationMinutes / 60 * 10) / 10} hours`);
    ...
    console.log({
      ...
      startReadable: verify.startTime ? new Date(verify.startTime).toLocaleString() : 'N/A',
      endReadable: verify.endTime ? new Date(verify.endTime).toLocaleString() : 'N/A',
    });

    // ✅ Get additional info for response and email
    const faculties = await getFaculties();
    const rooms = await getRooms();
    const faculty = faculties.find((f) => f.id === facultyId);
    const room = rooms.find((r) => r.id === roomId);

    // ✅ ENHANCED: Prepare session data for email (matching your existing function structure)
    ...
    // ✅ ENHANCED: Send invitation email using your existing function
    let emailResult = { ok: false, message: "", emailSent: false };
    
    console.log("📧 Preparing to send invitation email using existing sendInviteEmail function...");
    ...
    try {
      ...
      if (result.ok) {
        console.log("✅ Invitation email sent successfully using your existing function");
      } else {
    ...
    console.log(`   Dates: ${new Date(finalStartTime).toLocaleDateString()} - ${new Date(finalEndTime).toLocaleDateString()}`);
    console.log(`   Email sent: ${emailResult.ok ? '✅ Yes' : '❌ No'}`);
