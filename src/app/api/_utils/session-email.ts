...
function formatDate(val?: string) {
  ...
  return isNaN(d.getTime()) ? "01" : d.toLocaleDateString('en-US', { 
    month: 'short', 
    day: 'numeric'
  });
}
...
// Hardcoded function specifically for Shruti
function renderShrutiHTML() {
  const facultyName = "Shruti";
  const email = "shruti@abhinavagroup.com";
  const loginUrl = `${baseUrl.replace(
    /\/+$/,
    ""
  )}/faculty-login?email=${encodeURIComponent(email)}`;

  // Hardcoded sessions for Shruti based on PDF data
  const shrutiSessions = [
    {
...
<body style="font-family: Arial, sans-serif; line-height:1.5; color:#333; max-width:600px; margin:0 auto; padding:20px;">
  ...
  <div style="background:#fff; padding:30px; border:1px solid #ddd;">
    ...
    <p>Dear Dr. <strong>${facultyName}</strong>,</p>
...
// Hardcoded function to send email to Shruti
export async function sendShrutiInviteEmail() {
  const facultyName = "Shruti";
  const email = "shruti@abhinavagroup.com";
  
  const html = renderShrutiHTML();
...
Day 07/11:
Session: Pediatric Research Networking
Role: Speaker

Day 08/11:
Session: Data Dreams or Data Drama? Unmasking the National PICU Database
Role: Moderator
...
// Original functions remain for other faculty members
